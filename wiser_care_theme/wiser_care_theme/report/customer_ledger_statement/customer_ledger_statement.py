			...
			if filters.get("group_by") != _('Group by Voucher (Consolidated)'):
				...
				if gle.voucher_type == "Sales Invoice":
					...
					sales_invoice_item = frappe.db.sql("""
						...
						where parent=%s 
						order by idx
						""",gle.voucher_no, as_dict=1)
					if sales_invoice_item:
						for sii in sales_invoice_item:
							entries.append(frappe._dict(sii))
					#entries.append(_dict(frappe.db.get_list('Sales Invoice Item',fields=['item_name', 'qty', 'rate', 'amount'],filters={'parent': gle.voucher_no}, group_by='idx')))
